                    ...
                    if key not in found:
                        ...
                        vulns.append(make_vuln(
                            ...
                            evidence    = (
                                ...
                                f"Error detectado: {re.search(ep, body_lower).group(0)[:100] if re.search(ep, body_lower) else ep}"
                            ),
                            fix         = (
...
async def _test_xss(
    ...
    async def check(param: str, payload: str):
        async with sem:
            ...
            # Verificar si el payload aparece sin escape en el body
            body = resp.text

            # Buscar el payload o una forma de ejecución del mismo
            indicators = [
                payload,
                "<script>alert",
                "onerror=alert",
                ...
                if ind.lower() in body.lower():
                    if param not in found:
                        ...
                        vulns.append(make_vuln(
                            ...
                            evidence    = (
                                f"Payload: {payload[:80]}\n"
                                f"Reflejado en body (HTTP {resp.status})"
                            ),
                            fix         = (
